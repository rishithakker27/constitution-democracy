...
\significancestatement{After Egypt's 2011 uprising, reformers wrote executive constraints and judicial independence into the new constitution. After Lebanon's civil war, the Taif Agreement encoded confessional power-sharing. Neither produced durable democracy. This paper provides systematic evidence for why: when a machine learning model is trained on nearly 17,000 country-year observations, the constitutional provisions that best predict democracy are not separation of powers or judicial independence---the classical Madisonian safeguards---but political competition, institutional accountability, and civil liberties. These findings illuminate a fundamental de facto versus de jure divide: writing the right provisions into constitutional text is insufficient when the political conditions that make those provisions enforceable are absent. The constitutional gap between what a country's text promises and what it delivers is itself a measurable leading indicator of democratic backsliding.}
...
\begin{abstract}
What does constitutional text actually tell us about democracy? We address this question using a machine learning framework applied to 17,390 country-year observations spanning 1789--2023. We score each country-constitution dyad on 14 structural dimensions using an LLM-assisted typology applied to the Comparative Constitutions Project (CCPCNC v5), then train a gradient-boosted model (CatBoost) with country-blocked five-fold cross-validation to predict V-Dem Polyarchy scores from constitutional features alone. The model achieves a fold-average $R^2$ of $0.10 \pm 0.09$ and mean absolute error of $0.22$ on the 0--1 Polyarchy scale on entirely held-out countries. The most striking finding is in the feature rankings: Political Competition (10.7\%), Institutional Accountability (9.2\%), and Civil Liberties (8.7\%) are the top predictors, while the classical Madisonian safeguards---Executive Constraints (6.8\%), Judicial Independence (5.1\%), and Legislative Autonomy (4.5\%)---rank in the bottom half. We further construct a \emph{constitutional gap}---the difference between constitutionally predicted and actually achieved democracy---and show that this gap grows with constitutional youth: newly adopted constitutions are systematically associated with democratic under-delivery, while countries with older constitutional orders consistently outperform their text, suggesting that norms and institutional culture, not provisions alone, drive democratic performance.
\end{abstract}
...
Together, these findings cast doubt on constitutional engineering as a path to democracy. When Egypt's post-2011 constitution encoded judicial independence and executive constraints, or when Lebanon's elite negotiations under the Taif Agreement tried to redistribute power through constitutional text, the missing ingredient was not the right provisions---it was the de facto political competition and accountability that would have made those provisions meaningful.

The remainder of the paper is structured as follows. Section~2 reviews the relevant literature. Section~3 describes the data, the LLM-assisted typology, and the modeling strategy. Section~4 presents the Polyarchy prediction results and the constitutional gap analysis. Section~5 compares constitutional predictability across V-Dem's five democracy concepts. Section~6 examines how constitutional age shapes the gap between promise and performance. Section~7 traces how the 14 constitutional dimensions have evolved globally over the past century. Section~8 tests whether democratic culture---the variable that age is arguably a proxy for---explains additional variance, and discusses the limits of that test. Section~9 concludes.
...
Table~\ref{tab:model_performance} reports cross-validated performance across the five country-blocked folds. The fold-average $R^2$ is $0.10 \pm 0.09$, with per-fold values ranging from $-0.04$ (fold 4) to $0.21$ (fold 3); RMSE is $0.25$ and MAE is $0.22$ on the 0--1 Polyarchy scale. The in-sample $R^2$ of $0.89$ warrants a direct comment. The large gap between in-sample and out-of-fold performance could suggest overfitting, and with tree depth set to 3 some degree of within-sample memorisation is likely. But the more fundamental explanation is country heterogeneity. Constitutional provisions can be standardised---coded, dimensionalised, and compared across two centuries of drafting---yet the political experience of a state cannot. Each country arrives at its constitutional text through a specific history of conflict, bargaining, and institutional layering that shapes how those provisions are interpreted and enforced. A model trained on 140 countries learns those countries' constitutional fingerprints well; when asked to generalise to 35 entirely unseen countries, it faces not a modelling failure but an irreducible limit: constitutional text is a thin summary of what a state has committed to on paper, not of what it has actually become.
...
The modest OOF $R^2$ is interpretively significant. Constitutional text alone can predict roughly 10\% of the variance in democracy across unseen countries---non-trivially above zero, but far from deterministic. The $R^2$ swings from negative (fold 4, where held-out countries' constitutional profiles are poorly represented in training) to 0.21 (fold 3), suggesting that constitutional predictability is heterogeneous across the world's constitutional families.
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.75\linewidth]{outputs/feature_importances_kfold.png}
...
The constitutional gap---the difference between what a country's constitution implies about its democracy and what it actually achieves---is not evenly distributed across countries or time. Countries experiencing democratic backsliding show a characteristic pattern: the gap is near zero or slightly negative in earlier periods (actual democracy meets or slightly exceeds constitutional promise), then widens sharply as actual democratic performance deteriorates while constitutional text remains nominally unchanged. This pattern is consistent with backsliding episodes in Hungary (post-2010 Orbán constitutional consolidation), Venezuela (post-Chávez erosion), and Turkey (post-2016 coup attempt and constitutional referendum).

Egypt and Lebanon illustrate the limits of constitutional engineering from a different angle. Both countries adopted or substantially revised constitutional documents with strong democracy-promoting provisions in the aftermath of political crises---Egypt after the 2011 Arab Spring uprising, Lebanon through the 1989 Taif Agreement that ended the civil war. In Egypt, the post-Mubarak constitution included judicial independence provisions and executive constraints modeled on liberal democratic norms; the military coup of 2013 rendered them moot within two years. In Lebanon, the Taif Agreement redistributed confessional representation through constitutional text, yet actual democratic performance has remained persistently low and the state has deteriorated into functional collapse. In both cases, the gap between constitutional promise and democratic reality was large from the outset and never closed---characteristic of countries where constitutional provisions are adopted as signals to international audiences rather than as enforceable commitments.
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.75\linewidth]{outputs/robustness/feature_importances_comparison.png}
...
\subsection*{Which Countries Overperform and Underperform?}

The constitutional gap also reveals which countries' actual democracy diverges most from their text. Among the largest overperformers---actual democracy far exceeding constitutional promise---are Denmark, New Zealand, Canada, Belgium, and France. These are established democracies whose institutional cultures have built democratic performance well beyond what their formal provisions predict.

Among the largest underperformers are Nicaragua, Cuba, Afghanistan, Belarus, and Eritrea---cases where constitutions encode democratic commitments that ruling regimes have no intention of honoring. The gap for Nicaragua in 2023 is $+0.44$ Polyarchy points; for Cuba, $+0.41$. We return to a more detailed treatment of these patterns in future work.
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.75\linewidth]{outputs/06_age_and_inflation/constitution_inflation.png}
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.75\linewidth]{outputs/10_dimension_trends/dimension_change_bar.png}
...
The age finding invites a natural question: is age itself the mechanism, or is it a proxy for something that accumulates over time? The most direct candidate is democratic culture---the extent to which citizens and elites have internalized norms of democratic governance independently of what any formal document prescribes. If the age gradient reflects cultural consolidation, then a direct measure of culture should absorb most of the age effect and add substantially to the constitutional model.

We test this using the EIU Democratic Culture Index \cite{eiu_democracy_index}, a survey-based measure scored 0--10. Adding this single variable to the CatBoost model raises the out-of-fold $R^2$ from $0.07$ to $0.40$ on the subset of country-years with EIU coverage (2006--2023)---a lift of $+0.33$. An interaction specification reveals that the marginal effect of constitutional text on democracy varies sharply across culture levels: at low culture scores (EIU~$\approx 4$), a one-unit increase in constitutional score shifts predicted democracy by $0.63$ points; at high culture scores (EIU~$\approx 8$), that marginal effect collapses to $0.08$. Constitutions are nearly inert in low-culture environments regardless of how well they are written; in high-culture environments, performance has already outgrown the text. This is the same pattern the age analysis implies, now measured directly.

A simple 2$\times$2 typology classifies country-years by culture level and gap sign. ``Stuck autocracies'' (low culture, negative gap) show 90.9\% five-year persistence; ``ripe'' countries (high culture, negative gap) escape to a more democratic trajectory at a 16.7\% rate. Constitutional revision is rarely what tips ripe countries over---the culture was already there; the institution needed to catch up.

\textbf{Caveat: democracy and culture are co-constitutive.} The deeper limitation here is not merely measurement overlap but a theoretical one: democratic culture and democratic performance are mutually reinforcing over time. Countries that sustain democratic institutions develop the civic norms, elite expectations, and public commitments that make those institutions resilient; those norms in turn make further democratic consolidation easier. The causal arrow does not run cleanly from culture to democracy---it runs in both directions simultaneously. A country's EIU culture score is partly a record of how democracy has already played out within it. Using culture to predict democracy therefore risks attributing explanatory power to a variable that is itself a downstream product of the very process we are trying to explain. We therefore treat these results as descriptive rather than causal: the culture measure confirms that the residual left unexplained by constitutional text is structured and not random, consistent with the age-based evidence, but the intertwined nature of culture and democratic practice means culture cannot serve as a clean independent lever. Isolating culture's causal contribution would require exploiting exogenous shocks to civic norms---natural experiments that are rare and hard to identify at scale.

% ---------------------------------------------------------------
\section*{Conclusion}

This paper set out to test, systematically and at scale, whether constitutional text carries predictive information about democratic outcomes---and if so, which provisions actually matter. The answer to the first question is yes, but modestly: a gradient-boosted model trained on 14 constitutional dimension scores achieves an out-of-fold $R^2$ of roughly 0.10 on entirely unseen countries, well above chance but far from deterministic. The answer to the second question departs sharply from the Madisonian template. The provisions that best predict democracy are those governing Political Competition, Institutional Accountability, and Civil Liberties---not the classical separation-of-powers architecture of executive constraints, judicial independence, and legislative autonomy, which cluster near the bottom of the feature importance distribution regardless of whether the target is electoral or liberal democracy.

The constitutional gap---the signed difference between what a country's text implies and what it actually achieves---is not random noise. It is structured by constitutional age: documents that have survived for decades are embedded in countries that have consistently outperformed their text, while newly adopted constitutions, however rights-laden, are systematically associated with democratic under-delivery. The most parsimonious interpretation is that what makes constitutions work is not the provisions themselves but the institutional ecology---norms, expectations, enforcement capacity---that accumulates around them over time. Democratic culture, when measured directly, confirms this: it absorbs most of the variance that constitutional text cannot explain, though because democracy and culture develop together rather than independently, that explanatory power cannot be cleanly attributed to culture as a separable cause.

Taken together, the findings counsel skepticism about constitutional engineering as a path to democracy. The provisions that post-transition governments most reliably import---judicial independence clauses, executive constraint mechanisms, bicameral checks---are exactly the ones that carry the least predictive signal in the data. This does not mean such provisions are unimportant; it means that writing them into a document is not what makes them effective. What makes them effective is the de facto political environment that gives them teeth---the very thing that constitutional text, standing alone, cannot supply.
